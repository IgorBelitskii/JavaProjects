...
public class BooksAuthPublishOrm {
	...
	public String getAllBooks() {
		return em.createQuery("select b from Book b").getResultList().toString();
	}
	@Transactional
	...
	public String getAllAuthors() {
		...
		return em.createQuery("select a from Author a").getResultList().toString();
	}
	public String getBooksFromYear(int year) {
		
		return em.createQuery("select b from Book b where b.publishYear>="+year).getResultList().toString();
	}
	
public String getBooksBetweenYears(int yearFrom, int yearTo) {
		
		return em.createQuery("select b from Book b where b.publishYear BETWEEN "+yearFrom+" AND "+yearTo).getResultList().toString();
	}
